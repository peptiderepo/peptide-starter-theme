...
/**
 * Enqueue Beehiiv attribution script site-wide.
 *
 * Loads Beehiiv's attribution.js in the footer so all signup form placements
 * (subscribe page iframe, single-peptide inline form) are tracked correctly
 * under a single script include.
 *
 * Who calls it: wp_enqueue_scripts action (all front-end page loads).
 * Dependencies: none.
 *
 * @return void
 */
function peptide_starter_enqueue_beehiiv_attribution() {
	wp_enqueue_script( 'beehiiv-attribution', 'https://subscribe-forms.beehiiv.com/attribution.js', array(), null, true );
}
